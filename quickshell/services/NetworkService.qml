...
Singleton {
    ...
    property string userPreference: "wifi" // "auto", "wifi", "ethernet"
                ...
                const now = Date.now()                    
                    if (line.includes("PropertiesChanged") && line.includes("org.freedesktop.NetworkManager.AccessPoint")) {
                        if (line.includes("'Strength'") && root.activeAccessPointPath && line.includes(root.activeAccessPointPath)) {
                            root.parseSignalStrengthFromDbus(line)
                        }
                        return
                    }
        ...
        stdout: StdioCollector {
            onStreamFinished: {
                ...
                if (wifiInterface) {
                    ...
                    getWifiDevicePath.command = root.lowPriorityCmd.concat(["gdbus", "call", "--system", "--dest", "org.freedesktop.NetworkManager", "--object-path", "/org/freedesktop/NetworkManager", "--method", "org.freedesktop.NetworkManager.GetDeviceByIpIface", wifiInterface])
        ...
        stdout: StdioCollector {
            onStreamFinished: {
                ...
                if (match && match[1] !== '/') {
                    ...
                    checkWifiState.command = root.lowPriorityCmd.concat(["gdbus", "call", "--system", "--dest", "org.freedesktop.NetworkManager", "--object-path", match[1], "--method", "org.freedesktop.DBus.Properties.Get", "org.freedesktop.NetworkManager.Device", "State"])
    ...
    Process {
        id: checkWifiEnabled
        command: root.lowPriorityCmd.concat(["gdbus", "call", "--system", "--dest", "org.freedesktop.NetworkManager", "--object-path", "/org/freedesktop/NetworkManager", "--method", "org.freedesktop.DBus.Properties.Get", "org.freedesktop.NetworkManager", "WirelessEnabled"])
        running: false
    ...
    Process {
        id: requestWifiScan
        command: root.wifiInterface ? root.lowPriorityCmd.concat(["nmcli", "dev", "wifi", "rescan", "ifname", root.wifiInterface]) : []
        running: false
    ...
    Process {
        id: getWifiNetworks
        command: root.lowPriorityCmd.concat(["nmcli", "-t", "-f", "SSID,SIGNAL,SECURITY,BSSID", "dev", "wifi", "list", "ifname", root.wifiInterface])
        running: false
    ...
    function connectToWifi(ssid, password = "") {
        ...
        if (!password && root.ssidToConnectionName[ssid]) {
            ...
            wifiConnector.command = root.lowPriorityCmd.concat(["nmcli", "connection", "up", connectionName])
        } else if (password) {
            wifiConnector.command = root.lowPriorityCmd.concat(["nmcli", "dev", "wifi", "connect", ssid, "password", password])
        } else {
            wifiConnector.command = root.lowPriorityCmd.concat(["nmcli", "dev", "wifi", "connect", ssid])
        }
    ...
    function toggleWifiRadio() {
        ...
        wifiRadioToggler.command = root.lowPriorityCmd.concat(["nmcli", "radio", "wifi", targetState])
